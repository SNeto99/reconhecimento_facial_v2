...
class ZK(object):
    ...
    def enroll_face(self, user_id):
        """
        Cadastra uma face para um usuÃ¡rio especÃ­fico.
        
        Args:
            user_id (str): ID do usuÃ¡rio para cadastrar a face
            
        Returns:
            bool: True se o cadastro for bem sucedido, False caso contrÃ¡rio
        """
        try:
            # Desativa o dispositivo temporariamente
            self.disable_device()
            
            # Prepara o dispositivo para captura facial
            command = const.CMD_STARTENROLL
            command_string = pack('<L', int(user_id))
            cmd_response = self.__send_command(command, command_string)
            ...
            command = const.CMD_STARTIDENTIFY
            cmd_response = self.__send_command(command)
            
            if not cmd_response.get('status'):
                raise Exception("Falha ao iniciar captura facial")
            
            # Aguarda a conclusÃ£o do cadastro (timeout de 20 segundos)
            ...
            while time.time() < timeout:
                ...
                try:
                    ...
                    if data:
                        # Verifica se o cadastro foi concluÃ­do com sucesso
                        response = unpack('<L', data[:4])[0]
                        if response == const.CMD_ACK_OK:
                            return True
                        elif response == const.CMD_ACK_ERROR:
                            return False
                except socket.timeout:
                    continue
                
            raise Exception("Timeout ao aguardar cadastro facial")
    ...
    def init_face(self):
        ...
        """
        command = const.CMD_FACE_INIT
        cmd_response = self.__send_command(command)
        return cmd_response.get('status')

    def prepare_face(self, user_id):
        """
        Prepara o dispositivo para captura facial.
        
        Args:
            user_id (int): ID do usuário para preparar a captura
            
        Returns:
            bool: True se a preparação for bem sucedida, False caso contrário
        """
        command = const.CMD_PREPARE_FACE
        command_string = pack('<L', int(user_id))
        cmd_response = self.__send_command(command, command_string)
        return cmd_response.get('status')
    ...
    def set_face(self, user_id):
        ...
        Args:
            user_id (int): ID do usuário para iniciar a captura
            
        Returns:
            bool: True se a captura for bem sucedida, False caso contrário
        """
        command = const.CMD_SET_FACE
        command_string = pack('<L', int(user_id))
        cmd_response = self.__send_command(command, command_string)
        return cmd_response.get('status')

    def get_face(self, user_id):
        """
        Verifica se o cadastro facial foi concluído.
        
        Args:
            user_id (int): ID do usuário para verificar o cadastro
            
        Returns:
            bool: True se o cadastro foi concluído, False caso contrário
        """
        command = const.CMD_GET_FACE
        command_string = pack('<L', int(user_id))
        cmd_response = self.__send_command(command, command_string)
        return cmd_response.get('status')
    ...
    def get_face_status(self):
        """
        Verifica o status atual do cadastro facial
        
        Returns:
            dict: Dicionário com status da operação
        """
        command = const.CMD_GET_FACE
        cmd_response = self.__send_command(command)
        return {
